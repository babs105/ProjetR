...
class CaseWorkerList extends React.Component {
  state = {
    filter: 'all',
  };
  ...
  render() {
    ...
    const favoriteData = dataCase.filter(item => item.get('favorited') === true);
    const getItem = dataArray => dataArray.map(data => {
      const index = dataCase.indexOf(data);
      ...
      return (
        ...
        >
          <Avatar alt={data.get('name')} src={data.get('image')} className={classes.avatar} />
          <ListItemText primary={data.get('name')} secondary={data.get('occupation')} />
        </ListItem>
      );
    });
    return (
      <Fragment>
        ...
        >
          <div>
              ...
              <div className={classes.flex}>
                ...
                {addFn && (
                  <Tooltip title="Add New CaseWorker">
                    <IconButton className={classes.buttonAdd} onClick={() => addCaseWorker()} color="secondary" aria-label="Delete">
                      <Add />
                    </IconButton>
                  </Tooltip>
                )}
            ...
            <List>
              {filter === 'all' ? getItem(dataCase) : getItem(favoriteData)}
            </List>
          </div>
        </Drawer>
...
CaseWorkerList.defaultProps = {
  clippedRight: false,
  addCaseWorker: () => {},
  addFn: false,
};

export default withStyles(styles)(CaseWorkerList);
